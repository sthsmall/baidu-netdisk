...
class Server:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            print("连接成功，可用工具：", [tool.name for tool in response.tools])
...
class LLMClient:
    ...
    def get_response(self, messages: list[dict[str, str]]) -> str:
        ...
        #logging.info(f"request qianfan payload: {payload}")

        try:
            with httpx.Client(timeout=30.0) as client:
                response = client.post(url, headers=headers, json=payload)
                #logging.info(f"qianfan response: {response.text}")
